...
interface InstructionsViewProps {
  onNavigate: (view: 'landing' | 'main_menu' | 'calendar' | 'dashboard' | 'daily_agenda' | 'admin' | 'instructions' | 'modules' | 'accounts' | string) => void;
  onOpenProfile: () => void;
...
export function InstructionsView({ onNavigate, onOpenProfile, onOpenAffiliate, onOpenSupport, onOpenSubscription, onLogout, currentUser, isAdmin, currentLang: propLang }: InstructionsViewProps) {
    ...
    <div className="bg-brand text-white min-h-screen flex flex-col font-sans">
      ...
      <header className="bg-brand sticky w-full top-0 z-50 flex justify-between items-center px-5 py-2 shadow-sm relative">
            ...
            <>
              ...
              <div className="absolute left-0 top-12 w-auto min-w-[220px] max-w-[280px] bg-surface-container-high border border-outline-variant rounded-xl shadow-2xl overflow-hidden flex flex-col p-2 gap-1 z-50 text-left cursor-default" onClick={(e) => e.stopPropagation()}>
                <button onClick={() => { onNavigate('calendar'); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-on-surface hover:border-primary hover:bg-surface-container-highest cursor-pointer">
                  <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">calendar_month</span> {isEs ? 'Calendario' : isEn ? 'Calendar' : 'Calendário'}
                </button>
                <button onClick={() => { onNavigate('daily_agenda'); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-on-surface hover:border-primary hover:bg-surface-container-highest cursor-pointer">
                  <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">view_day</span> {isEs ? 'Agenda Diaria' : isEn ? 'Daily Agenda' : 'Agenda Diária'}
                </button>
                ...
                <button onClick={() => { onNavigate('accounts'); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-on-surface hover:border-primary hover:bg-surface-container-highest cursor-pointer">
                  <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">account_balance_wallet</span> {isEs ? 'Gestión de Cuentas' : isEn ? 'Account Management' : 'Gestão de Contas'}
                </button>
                <button onClick={() => { onOpenAffiliate(); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-on-surface hover:border-primary hover:bg-surface-container-highest cursor-pointer">
                  <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">groups</span> {isEs ? 'Mi Red' : isEn ? 'My Network' : 'Minha Rede'}
                </button>
                <button onClick={() => { onOpenProfile(); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-on-surface hover:border-primary hover:bg-surface-container-highest cursor-pointer">
                  <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">account_circle</span> {isEs ? 'Perfil' : isEn ? 'Profile' : 'Perfil'}
                </button>
                <button onClick={() => { if (onOpenSupport) onOpenSupport(); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-on-surface hover:border-primary hover:bg-surface-container-highest cursor-pointer">
                  <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">support_agent</span> {isEs ? 'Ayuda y Soporte' : isEn ? 'Help & Support' : 'Ajuda & Suporte'}
                </button>
                {isAdmin && (
                ...
                <button onClick={() => { onNavigate('instructions'); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent bg-surface-container-lowest text-primary font-bold shadow-sm cursor-pointer">
                  <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">menu_book</span> {isEs ? 'Instrucciones de uso' : isEn ? 'Instructions for use' : 'Instruções de uso'}
                </button>
                {(!currentUser || isAdmin) && (
                  <button onClick={() => { onNavigate('landing'); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-on-surface hover:border-primary hover:bg-surface-container-highest cursor-pointer">
                    <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">person_add</span> {isEs ? 'Registro' : isEn ? 'Sign Up' : 'Cadastro'}
                  </button>
                )}
                {onLogout && (
                  <button onClick={() => { onLogout(); setIsLogoMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all text-left border border-transparent text-red-400 hover:border-red-500 hover:bg-red-500/10 cursor-pointer">
                    <span className="material-symbols-outlined text-[20px] shrink-0 notranslate" translate="no">logout</span> {isEs ? 'Salir' : isEn ? 'Logout' : 'Sair'}
                  </button>
                )}
              </div>
            </>
        ...
        <div className="flex items-center gap-3 ml-auto z-10">
          <button onClick={() => onNavigate('main_menu')} className="w-10 h-10 rounded-full hover:bg-white/10 flex items-center justify-center transition-colors cursor-pointer">
             <span className="material-symbols-outlined text-white text-[28px] shrink-0 notranslate" translate="no">arrow_back</span>
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-auto p-5 pb-28 max-w-4xl mx-auto w-full">
        ...
        <div className="space-y-6">
          <div className="bg-surface-container-low border border-white/20 rounded-xl p-6 shadow-md backdrop-blur-sm">
            <h2 className="text-xl font-semibold mb-3 flex items-center gap-2 text-white">
              <span className="material-symbols-outlined shrink-0 notranslate" translate="no">app_registration</span>
          ...
          <div className="bg-surface-container-low border border-white/20 rounded-xl p-6 shadow-md backdrop-blur-sm">
            <h2 className="text-xl font-semibold mb-3 flex items-center gap-2 text-white">
              <span className="material-symbols-outlined shrink-0 notranslate" translate="no">menu</span>
          ...
          <div className="bg-surface-container-low border border-white/20 rounded-xl p-6 shadow-md backdrop-blur-sm">
            <h2 className="text-xl font-semibold mb-3 flex items-center gap-2 text-white">
              <span className="material-symbols-outlined shrink-0 notranslate" translate="no">add_circle</span>
            ...
            <ul className="list-decimal list-inside text-white/80 space-y-1 ml-2">
          ...
          <div className="bg-surface-container-low border border-white/20 rounded-xl p-6 shadow-md backdrop-blur-sm">
            <h2 className="text-xl font-semibold mb-3 flex items-center gap-2 text-white">
              <span className="material-symbols-outlined shrink-0 notranslate" translate="no">account_balance_wallet</span>
            ...
            <ul className="list-decimal list-inside text-white/80 space-y-1 ml-2">
              ...
              <li>{isEs ? 'Cree Metas Financieras (ej: "Viaje", "Fondo de Emergencia") y registre sus depósitos mensuales.' : isEn ? 'Create Financial Goals (e.g. "Travel", "Emergency Fund") and log monthly deposits.' : 'Crie Metas Financeiras (ex: "Viagem", "Reserva de Emergência") e registre seus depósitos mensuales.'}</li>
            </ul>
          </div>

          <div className="bg-surface-container-low border border-white/20 rounded-xl p-6 shadow-md backdrop-blur-sm">
            <h2 className="text-xl font-semibold mb-3 flex items-center gap-2 text-white">
              <span className="material-symbols-outlined shrink-0 notranslate" translate="no">groups</span>
